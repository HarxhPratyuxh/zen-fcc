...
@pytest.mark.asyncio
async def test_openrouter_lists_model_ids_via_openai_compat() -> None:
    config = ProviderConfig(api_key="open-router-key")
    with patch("providers.openai_compat.AsyncOpenAI"):
    ...
    with patch.object(
        ...
        return_value=SimpleNamespace(
            data=[
                SimpleNamespace(id="minimax-m2.5-free"),
                SimpleNamespace(id="claude-3-5-sonnet"),
            ]
        ),
    ):
        assert await provider.list_model_ids() == frozenset(
            {"minimax-m2.5-free", "claude-3-5-sonnet"}
        )
